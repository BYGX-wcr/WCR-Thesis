...
\item 第二章：带权拥塞控制。介绍典型的基于AIMD的带权拥塞控制算法。
\item 第三章：实验与分析。通过仿真和拟真实验来分析四种基于AIMD的带权拥塞控制算法。
\item 第四章：讨论与启示。从总体上讨论实验中分析出来的现象与规律。
\item 第五章：相关工作。介绍相关工作的具体情况。
\item 第六章：总结。总结本篇论文的成果。
\end{enumerate}
